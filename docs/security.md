...
| 管理員查詢結果 | 只存在記憶體，**不寫任何暫存檔、不寫快取、不寫 log** |
...
網路的管理員擴充只回報**中繼資料**：誰開了幾條連線、哪些 port 在監聽、
連線狀態的統計。
...
sysview 不建立任何暫存檔。沒有 `/tmp/sysview.json`、沒有 debug dump、
沒有 world-readable 的快取。管理員資料的生命週期就是 TUI 的記憶體。
